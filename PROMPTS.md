...
**Поведение агента при старте с нуля:** сначала запросить **настройки** и **геймдизайн** (идея, механики, экраны). Настройки: режим, платформа(ы), ориентация, стиль, разрешение, input, тогглы, Figma/ComfyUI. **Если выбран стиль из Figma** — дополнительно запросить ссылку на файл/фрейм Figma. Не стартовать без запроса настроек и геймдизайна (анти-паттерн). Планирование (GAME_DESIGN + DEV_PLAN) выполнять в **Plan mode**; после подтверждения плана — реализация.
...
```
Проект уже настроен. Посмотри DEV_CONFIG.md, DEV_STATE.md, AGENT_MEMORY.md и DEV_PLAN.md — продолжи разработку с того места, где остановились.
```

### Сменить / уточнить режим

```
Переключи режим разработки на [Прототип / Стандартный / Быстрый / Профи]. Обнови DEV_CONFIG.md.
```

### Записать в память агента

```
Запомни: [важная информация, предпочтение, решение, конвенция]. Запиши в AGENT_MEMORY.md.
```
...
```
Покажи текущее состояние сцены и сделай скриншот. Обнови DEV_STATE.md.
```
...
5. Обнови DEV_STATE (микро-план, 📈 прогресс, блокеры, скрин) + DEV_PLAN + текущую итерацию в Docs/DEV_LOG/.
```
